...
export default function TemplatesPage() {
  ...
  // ── usage stats per template (จาก branch_evaluations) ──
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      // คำนวณ usage stats per template
  ...
  const create = async () => {
    ...
    window.location.href = `/admin/branch-eval/templates/${d.id}`
  }
  ...
  return (
    <div className="p-4 lg:p-6 max-w-6xl mx-auto space-y-4 pb-32">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {templates.map((t: any) => (
            <Link key={t.id} href={`/admin/branch-eval/templates/${t.id}`}
              ...
              {/* ── Usage stats: ใช้ไปกี่ฟอร์ม / ส่งถึงกี่หัวหน้า / คะแนนเฉลี่ย ── */}
