...
export default function EvaluationsListPage() {
  ...
  // silent=false: initial (skeleton) / silent=true: auto-refresh (ไม่กระตุก)
  ...
  // auto-refresh ทุก 30 วินาที (silent) + เมื่อสลับ tab กลับมา
  useEffect(() => {
          ...
          <Link key={ev.id} href={`/admin/branch-eval/evaluations/${ev.id}`}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-500 truncate mt-0.5">
                ...
                {ev.evaluatee && <> · <User size={9} className="inline text-indigo-500" /> <span className="text-indigo-700 font-bold">ประเมิน {ev.evaluatee.first_name_th} {ev.evaluatee.last_name_th}{ev.evaluatee.nickname && ` (${ev.evaluatee.nickname})`}</span></>}
              </p>
            </div>
            {ev.status !== "draft" && (
