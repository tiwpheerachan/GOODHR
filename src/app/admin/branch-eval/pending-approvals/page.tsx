// ─── reuse component จากฝั่ง app — admin link จะ navigate มาที่ pageนี้ ───
// แต่ component พา navigate ไป /app/branch-eval/manage/evaluations/[id]
// ในฝั่ง admin เราต้องการลิงก์ไป /admin/branch-eval/evaluations/[id] แทน
"use client"
import { useEffect, useState, useMemo } from "react"
import Link from "next/link"
import {
  BadgeCheck, ClipboardCheck, Loader2, Search, Calendar, User,
...
export default function AdminPendingApprovalsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "target_me">("all")
      ...
      <div className="bg-gradient-to-br from-amber-500 via-orange-500 to-rose-500 rounded-2xl p-5 text-white shadow-md relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] opacity-90 mt-0.5">ฟอร์มประเมินสาขาที่รอตรวจและตัดสินใจ</p>
          </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 relative">
          <div className="bg-white/15 backdrop-blur-sm rounded-xl p-2.5">
            ...
            <p className="text-2xl font-black mt-0.5">{list.length}</p>
            <p className="text-[10px] opacity-70">ฟอร์ม</p>
          </div>
          <div className="bg-white/15 backdrop-blur-sm rounded-xl p-2.5">
            <p className="text-[10px] uppercase opacity-80 font-bold">ส่งถึงคุณตรง</p>
