...
const CAL_BG: Record<string, string> = {
  ...
  early_out: "bg-orange-100  text-orange-600",
  leave:     "bg-indigo-100  text-indigo-600",
  wfh:       "bg-teal-100    text-teal-700",
}
  ...
  return (
    <>
      <style>{`
        ...
        .att-bg { background: linear-gradient(160deg,#f0fdf9 0%,#e0f2fe 50%,#f0fdf4 100%); min-height:100vh; }
        .glass-card { background:rgba(255,255,255,0.85); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,0.9); box-shadow:0 4px 24px rgba(0,0,0,0.06); }
        ...
        .ring-track { stroke:rgba(16,185,129,0.12); }
      ...
      <div className="att-bg pb-12">
        ...
        <div className="px-4 pt-6 pb-4 fade-up">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">ประวัติการเข้างาน</p>
        ...
        <div className="px-4 mb-4 fade-up-1">
          <div className="glass-card rounded-2xl flex items-center justify-between px-3 py-2.5">
            <button onClick={()=>setMonth(m=>subMonths(m,1))}
              className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-emerald-50 active:scale-90 transition-all">
            ...
            <button onClick={()=>setMonth(m=>addMonths(m,1))}
              ...
              className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-emerald-50 active:scale-90 transition-all disabled:opacity-25">
            ...
            <div className="glass-card rounded-3xl overflow-hidden border-red-100" style={{border:"1px solid rgba(254,202,202,0.8)"}}>
              ...
              <div className="divide-y divide-slate-50/80">
                ...
                {stats.late>periodStats.late && (
                  <div className="flex items-center gap-2 px-4 py-2.5 bg-sky-50/50">
                    <Info size={10} className="text-sky-400 flex-shrink-0"/>
                    <p className="text-[10px] text-sky-600">สาย {stats.late-periodStats.late} ครั้งหลังวันที่ 21 — จะนับในงวดถัดไป</p>
                  </div>
                )}
                <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50/60">
                  ...
                  <p className="text-[10px] text-slate-500">
                    ดูยอดหักจริงได้ที่{" "}
                    <Link href="/app/salary" className="text-emerald-600 font-bold underline">หน้าเงินเดือน →</Link>
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="glass-card rounded-3xl p-4">
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({length:getDaysInMonth(month)},(_,i) => {
                ...
                let cls="", sub=""
                if      (isT)    { cls="bg-gradient-to-br from-emerald-500 to-cyan-500 text-white shadow-md shadow-emerald-200"; sub=rec?(STATUS_TH[rec.status]||"").slice(0,2):"วันนี้" }
        ...
        {/* ── Record list ────────────────────────────────────────── */}
        <div className="px-4 space-y-2.5 fade-up-3">
          {loading ? (
              ...
              >
                <div className={`flex items-center gap-3 px-4 py-3.5 ${isAbsent&&isVirtual?"bg-red-50/30":""}`}>
                    ...
                    {!isVirtual ? (
                      <div className="flex items-center gap-2.5 text-xs">
                        <span className="flex items-center gap-1 text-slate-500">
                          <LogIn size={9} className="text-emerald-500"/>
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    {(r.ot_minutes||0)>0 && <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full">OT {r.ot_minutes}น.</span>}
                  </div>
                </div>
                ...
                {showActions && (
                  <div className={`flex divide-x ${isAbsent?"border-t border-red-100 divide-red-100":"border-t border-slate-100 divide-slate-100"}`}>
                    <Link href={`/app/leave/new?type=adjustment&date=${r.work_date}`}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-black text-blue-600 hover:bg-blue-50 transition-colors">
        ...
        {Object.keys(holidayMap).filter(d=>d.startsWith(monthPfx)).length>0 && (
          <div className="px-4 mt-4 fade-up-4">
            <div className="glass-card rounded-3xl p-4">
