...
export default function TableTab({ canSeeAll, canSeeTeam }: { canSeeAll: boolean; canSeeTeam: boolean }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setData(d)
        // เก็บรายชื่อพนักงานทั้งหมดไว้ (อัปเดตเฉพาะตอนยังไม่กรองพนักงาน = ได้ลิสต์ครบ)
        if (!filters.employee_id && d.facets?.employees) setEmpOptions(d.facets.employees)
      } else toast.error(d.error || "โหลดไม่สำเร็จ")
    } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [scope, start, end, filters.branch_name, filters.sales_channel, filters.brand, filters.category, filters.source, filters.employee_id])

  const rows = useMemo(() => {
    if (!data?.sales) return []
    const s = filters.search.trim().toLowerCase()
    let arr = !s ? data.sales : data.sales.filter((x: any) => {
    ...
    if (filters.product) arr = arr.filter((x: any) => x.product_name === filters.product)
    // sort
    ...
    return sorted
  }, [data, filters.search, filters.product, sortBy])

  const PER_PAGE = 50
  const totalPages = Math.ceil(rows.length / PER_PAGE)
  const slice = rows.slice(page * PER_PAGE, (page + 1) * PER_PAGE)
  useEffect(() => { setPage(0) }, [filters.search, filters.product, data])
  ...
  const facets = data?.facets || { branches: [], channels: [], brands: [], categories: [], sources: [], products: [], employees: [] }
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Advanced filters */}
      {showFilters && (
        <div className="bg-gradient-to-br from-indigo-50/40 to-purple-50/40 rounded-2xl border border-indigo-100 p-3 grid grid-cols-2 md:grid-cols-4 gap-2">
          {/* 👤 พนักงาน — ดูว่าใครขายอะไร */}
          <EmployeeSelect label="👤 พนักงาน" value={filters.employee_id} options={empOptions}
            onChange={(v: string) => setFilters(f => ({ ...f, employee_id: v }))}/>
          <FilterSelect label="🛍️ สินค้า" value={filters.product} options={facets.products} onChange={(v: string) => setFilters(f => ({ ...f, product: v }))}/>
...
function EmployeeSelect({ label, value, options, onChange }: any) {
  return (
    <div>
      <p className="text-[9px] font-black text-indigo-500 uppercase mb-1">{label}</p>
      <select value={value} onChange={e => onChange(e.target.value)}
        className="w-full bg-white border border-indigo-200 rounded-lg px-2 py-1.5 text-xs outline-none focus:border-indigo-400 font-bold text-indigo-700">
        <option value="">ทุกคน</option>
        {(options || []).map((o: { id: string; name: string }) => <option key={o.id} value={o.id}>{o.name}</option>)}
      </select>
    </div>
  )
}
